...
const getEmployerFolderName = appConfig.appRoot.getEmployerFolderName;
...
(async () => {
    try {
        ...
        // -------employer folder route-------
        const employerFiles = await utils._readdir(`./app/router/${getEmployerFolderName}`);
        employerFiles.forEach((file) => {
            if (!file || file[0] == ".") return;
            namedRouter.use("/", require(join(__dirname, file)));
        });
        ...
        if (!isProduction && process.env.SHOW_NAMED_ROUTES === "true") {
            ...
            const employerRouteList = namedRouter.getRouteTable("/employer");
            ...
            console.log("Employer Routes:", employerRouteList);
